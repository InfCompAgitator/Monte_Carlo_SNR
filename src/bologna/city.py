...
from src.bologna.streets import StreetGraph
import numpy as np
...
class City:
    ...
    def plot_footprints(self, fig, ax):
        for fp in self.footprints_centers:
            ax.plot(fp.center_coords.x, fp.center_coords.y, marker='o', markersize=3, markerfacecolor='none', markeredgecolor='g')
    ...
    def set_footprints_coords(self):
        ...
        for idx, street in enumerate(self.street_graph.streets):
            ...
            if (current_coord - street.start_node_coords).norm() < street.length:
                new_fp = Footprint(Coords3d(start_coords_x, start_coords_y, 0), street_id=idx, start_coords=Coords3d(start_coords_x - d_x_1, start_coords_y - d_y_1, 0), end_coords=Coords3d(start_coords_x + d_x_1, start_coords_y + d_y_1, 0))
                ...
                new_fp = Footprint((street.start_node_coords + street.end_node_coords) / 2, street_id=idx, start_coords=street.start_node_coords, end_coords=street.end_node_coords)
                new_fp.side_coords= normal_vector * STREET_WIDTH + new_fp.center_coords
                street.fps.append(new_fp)
                self.footprints_centers.append(new_fp)
            prev_fp = new_fp
            for i in range(n_foot_prints - 1):
                ...
                if (current_coord - street.start_node_coords).norm() < street.length:
                    new_fp = Footprint(current_coord.copy(), street_id=idx, start_coords=current_coord - Coords3d(d_x_1, d_y_1, 0), end_coords=current_coord + Coords3d(d_x_1, d_y_1, 0))
                    new_fp.side_coords= normal_vector * STREET_WIDTH + new_fp.center_coords
                    self.footprints_centers.append(new_fp)
                    street.fps.append(new_fp)
                else:
                    new_fp = Footprint((prev_fp.center_coords+street.end_node_coords)/2, street_id=idx, start_coords=prev_fp.center_coords, end_coords=street.end_node_coords)
                    new_fp.side_coords= normal_vector * STREET_WIDTH + new_fp.center_coords
                    self.footprints_centers.append(new_fp)
                    street.fps.append(new_fp)
                prev_fp = new_fp
    def build_visibility_graph(self, save=False, load=True):
        ...
        if not load:
            ...
            for loc_idx, dbs_loc in tqdm.tqdm(enumerate(self.possible_locs), total=len(self.possible_locs)):
                ...
                for idx in idxs:
                    self.visibility_graph.add_edge(loc_idx + n_footprints, idx, fp_obj=self.footprints_centers[idx])
        else:
            with open(VIS_GRAPH_FILE, 'rb') as file:
                self.visibility_graph = pickle.load(file)
        if save:
            with open(VIS_GRAPH_FILE, 'wb') as _file:
    ...
    def calculate_beam_access_probability(self, fp_idx=200, dbs_loc_idx=565):
        ...
        street_density = self.street_graph.streets[street_id].expected_density
        fp = self.footprints_centers[fp_idx]
        dbs_coords = self.possible_locs[dbs_loc_idx]
        ap = calculate_beam_access_probability(street_density, fp, dbs_coords, BEAM_LENGTH, STREET_WIDTH)
